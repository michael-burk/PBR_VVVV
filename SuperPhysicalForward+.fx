...
#include "C:\vvvv\vvvv_50beta35.5_x64\packs\ForwardPlus\nodes\dx11\forwardplus.fxh"
#endif
...
float4 doLighting(float4 PosW, float3 N, float3 V, float4 TexCd, float4 position){
	...
	uint numlDiff, dummyDiff;lDiff.GetDimensions(numlDiff, dummyDiff);
	uint numlAtt0, dummylAtt0;lAtt0.GetDimensions(numlAtt0, dummylAtt0);
	uint numlAtt1, dummylAtt1;lAtt1.GetDimensions(numlAtt1, dummylAtt1);
	uint numlAtt2, dummylAtt2;lAtt2.GetDimensions(numlAtt2, dummylAtt2);
	uint numLights,lightCount;lightType.GetDimensions(numLights,lightCount);
    ...
    uint lightCountForward  = LightGrid[flatIndex].y;

	lightCountForward  = min(lightCount,lightCountForward);
   
	if(forwardPlus){
		numLights = lightCountForward;
	}
	for(uint i = 0; i< numLights; i++){

		uint lightIndex = LightIndexList[startOffset + i];
		if(!forwardPlus) lightIndex = i;
		float3 lightToObject = lPos[lightIndex] - PosW.xyz;
		float3 L = normalize(lightToObject);
		float lightDist = length(lightToObject);
		float falloff = pow(saturate(lightRange[lightIndex%lightCountForward]-lightDist),1.5);
		...
		switch (lightType[i]){
			...
			case 0:
				...
				viewPosition = mul(PosW, LightVP[lightIndex]);
				...
				&& (saturate(projectTexCoordZ) == projectTexCoordZ)){
					shadow = saturate(calcShadowVSM(lightDist, lightRange[lightIndex%lightCountForward],projectTexCoord,shadowCounter));	
				} else {
					shadow = 1;
				}
					if(useShadow[i]){
							shadowCounter++;
							finalLight.xyz += cookTorrance(V, L, N, albedo.xyz, lDiff[lightIndex%numlDiff].xyz, lAmbient[lightIndex%numlDiff].xyz,
											  lerp(1.0,saturate(shadow),falloff).x, 1.0, 1, lightDist, lAtt0[lightIndex%numlAtt0], lAtt1[lightIndex%numlAtt1], lAtt2[lightIndex%numlAtt2], F0, lAtt0[lightIndex%numlAtt0], texRoughness, metallicT, aoT,iridescenceColor);
					} else {
					       	finalLight.xyz += cookTorrance(V, L, N, albedo.xyz, lDiff[lightIndex%numlDiff].xyz, lAmbient[lightIndex%numlDiff].xyz,
											  1.0, 1.0, 1.0, lightDist, lAtt0[lightIndex%numlAtt0], lAtt1[lightIndex%numlAtt1], lAtt2[lightIndex%numlAtt2], F0, lAtt0[lightIndex%numlAtt0], texRoughness, metallicT, aoT,iridescenceColor);
					}
				lightCounter ++;
				break;
			...
			case 1:
				...
				&& (saturate(projectTexCoordZ) == projectTexCoordZ)){
					...
					if(tXS+tYS > 4) falloffSpot = lightMap.SampleLevel(g_samLinear, float3(projectTexCoord, spotLightCount), 0 ).rgb;
				...
				if(useShadow[i]){
						...
						float attenuation = lAtt0[lightIndex%numlAtt0] / pow(lightDist,lAtt1[lightIndex%numlAtt1]);
						finalLight.xyz += cookTorrance(V, L, N, albedo.xyz, lDiff[lightIndex%numlDiff].xyz, lAmbient[lightIndex%numlDiff].xyz,
						lerp(1.0,saturate(shadow),falloff).x, falloffSpot*falloff, falloff, lightDist, lAtt0[lightIndex%numlAtt0], lAtt1[lightIndex%numlAtt1], lAtt2[lightIndex%numlAtt2], F0, attenuation, texRoughness, metallicT, aoT, iridescenceColor);
					
				} else {
						float attenuation = lAtt0[lightIndex%numlAtt0] / pow(lightDist,lAtt1[lightIndex%numlAtt1]);
						finalLight.xyz += cookTorrance(V, L, N, albedo.xyz, lDiff[lightIndex%numlDiff].xyz, lAmbient[lightIndex%numlDiff].xyz,
						1.0, falloffSpot, falloff, lightDist, lAtt0[lightIndex%numlAtt0], lAtt1[lightIndex%numlAtt1], lAtt2[lightIndex%numlAtt2], F0, attenuation, texRoughness, metallicT, aoT, iridescenceColor);
				}
				lightCounter ++;
				spotLightCount++;
				break;
			...
			case 2:
				...
				if(useShadow[lightIndex]){
					...
					for(int p = 0; p < 6; p++){
						...
						float4x4 LightVPNew = mul(LightV[p + lightCounter],LightPcropp);
						...
						&& (saturate(projectTexCoordZ) == projectTexCoordZ)){
							...
							viewPosition = mul(PosW, LightVP[p + lightCounter]);

							projectTexCoord.x =  viewPosition.x / viewPosition.w / 2.0f + 0.5f;
				   			projectTexCoord.y = -viewPosition.y / viewPosition.w / 2.0f + 0.5f;
							projectTexCoordZ = viewPosition.z / viewPosition.w / 2.0f + 0.5f;
							
							shadow += saturate(calcShadowVSM(lightDist, lightRange[lightIndex%lightCountForward], projectTexCoord,p+shadowCounter));

						}
					}
							float attenuation = lAtt0[lightIndex%numlAtt0] / pow(lightDist,lAtt1[lightIndex%numlAtt1]);
							finalLight.xyz += cookTorrance(V, L, N, albedo.xyz, lDiff[lightIndex%numlDiff].xyz, lAmbient[lightIndex%numlDiff].xyz,
							lerp(1,saturate(shadow),falloff).x, 1.0, falloff, lightDist, lAtt0[lightIndex%numlAtt0], lAtt1[lightIndex%numlAtt1], lAtt2[lightIndex%numlAtt2], F0, attenuation, texRoughness, metallicT, aoT, iridescenceColor);
						    ...
						    float attenuation = lAtt0[lightIndex%numlAtt0] / pow(lightDist,lAtt1[lightIndex%numlAtt1]);
							finalLight.xyz += cookTorrance(V, L, N, albedo.xyz, lDiff[lightIndex%numlDiff].xyz, lAmbient[lightIndex%numlDiff].xyz,
							1, 1, falloff, lightDist, lAtt0[lightIndex%numlAtt0], lAtt1[lightIndex%numlAtt1], lAtt2[lightIndex%numlAtt2], F0, attenuation, texRoughness, metallicT, aoT, iridescenceColor);
...
float4 PS_PBR(vs2ps In, float4 svPos : SV_POSITION): SV_Target
{	
	return doLighting(In.PosW, normalize(In.NormW), In.V, In.TexCd, svPos);
}
...
{	
	...
	return doLighting(In.PosW, Nb, In.V, In.TexCd, float4(0,0,0,0));
